...
measures the protocol gap on the same model at $0.8815 - 0.7902 = +0.0913$, several times the
pooled $+0.018$ measured here (Section~\ref{sec:pooled}), which supports treating our figure as
a floor. Second, its Tables~4 and~5 hold PR-AUC fixed at $0.8815$ and $0.7902$ across three very
...
same code --- shifted PR-AUC by $+0.033$ on average, \emph{larger} than the pooled
random-versus-chronological effect of $+0.018$ (Section~\ref{sec:pooled}). \emph{Choosing $20\%$
rather than $15\%$ as the test tail moves the number at least as much as abandoning
chronological evaluation altogether.} Part of the mechanism is visible in the windows: the $66/14/20$ test partition
...
Shuffling raises five of six configurations at this cut point --- up to $+0.06$ for the
synthetic-data pipeline, whose fabricated frauds land next to their real neighbours in the test
fold. GraphSAGE is the instructive exception --- shuffling scrambles the temporal coherence its
neighbourhoods relied upon. These six runs are a \emph{single} cut point, and
Section~\ref{sec:pooled} shows why they should not be read as the protocol effect on their own.

\subsection{Pooling the protocol measurements}\label{sec:pooled}
Across every paired run in this report where the split assignment is the only variable --- the
six configurations above, XGBoost with class weighting at the same fractions, and the three
configurations of Section~\ref{sec:sangrep} at $66/14/20$ --- ten measurements are available:

\begin{center}\small
\begin{tabular}{lccc}
      \toprule
      random $-$ chronological & $n$ & positive & mean\\
      \midrule
      at $70/15/15$ & 7 & 6 of 7 & $+0.0283$\\
      at $66/14/20$ & 3 & 1 of 3 & $-0.0054$\\
      \midrule
      \textbf{pooled} & \textbf{10} & \textbf{7 of 10} & $\mathbf{+0.0182}$\\
      \bottomrule
\end{tabular}
\end{center}

\noindent The pooled effect carries a 95\% confidence interval of $[-0.0084,\, +0.0448]$.

The interval includes zero ($t = 1.55$; one-sided sign test $p = 0.172$), and the estimate is
not stable across cut points. \emph{These data establish the direction, not a reliable
magnitude.} Two notes bound the reading: our shuffled arm carries the 517 duplicate rows that
shuffling separates across the train/test boundary (Section~\ref{sec:protocol}), which biases
the estimate \emph{upward}; and it excludes the resample-before-split channel entirely, which
the literature audit identifies as the larger one. Neither changes the reason this report
evaluates chronologically, which is a validity argument rather than a performance one: a random
split fits the model on transactions that occurred after those it is scored on, and no deployed
system can do that. The full argument, with sources, is
\code{reports/why\_chronological\_split.pdf}.
...
split. The pooled effect we measure ($+0.018$, Section~\ref{sec:pooled}) is therefore a
